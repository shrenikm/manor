...
Invoked by the hatchling build hook on uv pip install --no-cache-dir -e . and can also be run directly:

    python scripts/compile_messages.py

LCM python modules are emitted in-place under the package tree so they're importable as
manor.common.definitions.lcmtypes.<name>. The entire lcmtypes/ directory is wiped and regenerated on each run.

Cap'n Proto schemas are validated by loading each top-level wrapper schema with a fresh SchemaParser.

When javac / jar / lcm.jar are all present, Java bindings for the same LCM schemas are also compiled and bundled into
build/java/manor_lcmtypes.jar for use with lcm-spy (exposed via the manor_lcm_spy console script). This step is
optional -- it is soft-skipped with a warning when any of the tools is missing, so headless / install-only environments
still succeed.
"""
...
def _compile_lcm_schemas() -> None:
    ...
    Each .lcm file declares package manor.common.definitions.lcmtypes; so lcm-gen with --ppath src/ drops its output
    directly into the package tree.
    """
...
def _find_lcm_jar() -> Path | None:
    """
    Locate the lcm.jar that ships alongside the installed lcm Python package, falling back to a couple of standard
    system paths.
    """
    try:
        import lcm as _lcm
...
def _compile_lcm_java_bindings() -> None:
    ...
    Soft-skips (prints a warning and returns) if javac, jar, lcm-gen, or lcm.jar are missing -- the jar is only needed
    to decode Aegis messages inside lcm-spy, so a missing Java toolchain must not break the rest of the install.
    """
