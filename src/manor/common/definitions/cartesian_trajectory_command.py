"""
Cartesian-space trajectory command container.

Wraps a multi-step task-space trajectory as a tagged-union of either
CartesianPoseTrajectory or CartesianTwistTrajectory. Exactly one
variant must be set; construction with zero or more than one raises
...
class _CapnpField(StrEnum):
    HEADER = "header"
    CARTESIAN_POSE_TRAJECTORY = "cartesianPoseTrajectory"
    CARTESIAN_TWIST_TRAJECTORY = "cartesianTwistTrajectory"


@attr.frozen
class CartesianTrajectoryCommand(DefinitionBase):
    """
    A Cartesian-space trajectory command. Exactly one of the variant fields must be non-None.
    """

    header: TimestampHeader
    cartesian_pose_trajectory: CartesianPoseTrajectory | None = None
    cartesian_twist_trajectory: CartesianTwistTrajectory | None = None
    ...
    def get_capnp_schema(cls) -> CapnpStructSchema:
        return load_versioned_schema("cartesian_trajectory_command.capnp").VersionedCartesianTrajectoryCommand

    @classmethod
    @override
    def get_lcm_class(cls) -> type:
        return lcmt_cartesian_trajectory_command
    ...
    def _active_variant_field(self) -> attr.Attribute:
        for f in self._variant_fields():
            if getattr(self, f.name) is not None:
                return f
        raise InvalidDefinitionError("CartesianTrajectoryCommand has no active variant")
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if len(active) != 1:
            raise InvalidDefinitionError(
                f"CartesianTrajectoryCommand requires exactly one variant field to be set; got {len(active)}: {[f.name for f in active]}"
            )
    ...
    def from_capnp_v1(cls, reader: Any) -> Self:
        ...
        try:
            ...
            raise InvalidDefinitionError(f"Unknown cartesian_trajectory_command capnp arm {arm!r}") from e
    ...
    @override
    def to_lcm_message(self) -> lcmt_cartesian_trajectory_command:
        msg = lcmt_cartesian_trajectory_command()
    ...
    def from_lcm_message(cls, msg: Any) -> Self:
        ...
        if not (0 <= variant < len(variant_fields)):
            raise InvalidDefinitionError(f"Unknown cartesian_trajectory_command variant tag {variant}")
    ...
    def construct_default(cls) -> Self:
        return cls(
            ...
            cartesian_pose_trajectory=CartesianPoseTrajectory.construct_default(),
        )
