...
from manor.common.definitions.lcmtypes.lcmt_ee_positions_trajectory import (
    lcmt_ee_positions_trajectory,
)
...
@attr.frozen
class EEPositionsTrajectory(DefinitionBase):
    """
    A trajectory of EE generalized positions.
    """
    ...
    def __attrs_post_init__(self) -> None:
        if self.times.shape[0] == 0:
            raise InvalidDefinitionError(
                "EEPositionsTrajectory must contain at least one step (use None for an absent trajectory)"
            )
        if self.times.shape[0] != self.ee_positions_array.shape[0]:
            raise InvalidDefinitionError(
                f"EEPositionsTrajectory.times length ({self.times.shape[0]}) "
    ...
    def get_capnp_schema(cls) -> CapnpStructSchema:
        return load_versioned_schema("ee_positions_trajectory.capnp").VersionedEEPositionsTrajectory

    @classmethod
    @override
    def get_lcm_class(cls) -> type:
        return lcmt_ee_positions_trajectory
    ...
    @override
    def to_lcm_message(self) -> lcmt_ee_positions_trajectory:
        msg = lcmt_ee_positions_trajectory()
        msg.header = self.header.to_lcm_message()
        arr = np.ascontiguousarray(self.ee_positions_array, dtype=np.float64)
        ...
        msg.ee_positions_array = arr.tolist()
        return msg
