"""
Joint-space command container.

Wraps an instantaneous joint-level setpoint as a tagged-union of either
JointPositions or JointVelocities. Exactly one variant must be set;
construction with zero or more than one raises InvalidDefinitionError.
"""
...
@attr.frozen
class JointCommand(DefinitionBase):
    """
    A joint-space command. Exactly one of the variant fields must be non-None.
    """
    ...
    def get_capnp_schema(cls) -> CapnpStructSchema:
        return load_versioned_schema("joint_command.capnp").VersionedJointCommand

    @classmethod
    @override
    def get_lcm_class(cls) -> type:
        return lcmt_joint_command
    ...
    def _active_variant_field(self) -> attr.Attribute:
        for f in self._variant_fields():
            if getattr(self, f.name) is not None:
                return f
        raise InvalidDefinitionError("JointCommand has no active variant")
    ...
    def from_capnp_v1(cls, reader: Any) -> Self:
        ...
        try:
            ...
            raise InvalidDefinitionError(f"Unknown joint_command capnp arm {arm!r}") from e
    ...
    @override
    def to_lcm_message(self) -> lcmt_joint_command:
        msg = lcmt_joint_command()
    ...
    def from_lcm_message(cls, msg: Any) -> Self:
        ...
        if not (0 <= variant < len(variant_fields)):
            raise InvalidDefinitionError(f"Unknown joint_command variant tag {variant}")
