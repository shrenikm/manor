...
class DefinitionBase(ISerializable, ILcmMessage):
    ...
    Subclasses MUST implement:
        ...
        to_capnp_current(self, builder) -> None
            Fill the current-version struct builder.
        from_capnp_v{N}(cls, reader) -> Self
    ...
    def from_versioned_capnp(cls, versioned_reader: Any) -> Self:
        ...
        handler = getattr(cls, f"from_capnp_{arm}", None)
        if handler is None:
    ...
    def deserialize(cls, data: bytes) -> Self:
        try:
            ...
            with schema.from_bytes(data) as msg:
                return cls.from_versioned_capnp(msg)
        except SerializationError:
            raise
        except Exception as e:
