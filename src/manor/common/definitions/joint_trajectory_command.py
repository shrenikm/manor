"""
Joint-space trajectory command container.

Wraps a multi-step joint-level trajectory as a tagged-union of either
JointPositionsTrajectory or JointVelocitiesTrajectory. Exactly one
...
@attr.frozen
class JointTrajectoryCommand(DefinitionBase):
    """
    A joint-space trajectory command. Exactly one of the variant fields must be non-None.
    """
    ...
    def get_capnp_schema(cls) -> CapnpStructSchema:
        return load_versioned_schema("joint_trajectory_command.capnp").VersionedJointTrajectoryCommand

    @classmethod
    @override
    def get_lcm_class(cls) -> type:
        return lcmt_joint_trajectory_command
    ...
    def _active_variant_field(self) -> attr.Attribute:
        for f in self._variant_fields():
            if getattr(self, f.name) is not None:
                return f
        raise InvalidDefinitionError("JointTrajectoryCommand has no active variant")
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if len(active) != 1:
            raise InvalidDefinitionError(
                f"JointTrajectoryCommand requires exactly one variant field to be set; got {len(active)}: {[f.name for f in active]}"
            )
    ...
    def from_capnp_v1(cls, reader: Any) -> Self:
        ...
        try:
            ...
            raise InvalidDefinitionError(f"Unknown joint_trajectory_command capnp arm {arm!r}") from e
    ...
    @override
    def to_lcm_message(self) -> lcmt_joint_trajectory_command:
        msg = lcmt_joint_trajectory_command()
    ...
    def from_lcm_message(cls, msg: Any) -> Self:
        ...
        if not (0 <= variant < len(variant_fields)):
            raise InvalidDefinitionError(f"Unknown joint_trajectory_command variant tag {variant}")
    ...
    def construct_default(cls) -> Self:
        return cls(
            ...
            joint_positions_trajectory=JointPositionsTrajectory.construct_default(),
        )
