"""
End-effector (gripper) trajectory command container.

Wraps a multi-step gripper-side trajectory as a tagged-union of either
EEPositionsTrajectory or EEVelocitiesTrajectory. Exactly one variant
must be set; construction with zero or more than one raises
...
class _CapnpField(StrEnum):
    HEADER = "header"
    EE_POSITIONS_TRAJECTORY = "eePositionsTrajectory"
    EE_VELOCITIES_TRAJECTORY = "eeVelocitiesTrajectory"


@attr.frozen
class EETrajectoryCommand(DefinitionBase):
    """
    An end-effector (gripper) trajectory command. Exactly one of the variant fields must be non-None.
    """

    header: TimestampHeader
    ee_positions_trajectory: EEPositionsTrajectory | None = None
    ee_velocities_trajectory: EEVelocitiesTrajectory | None = None
    ...
    def get_capnp_schema(cls) -> CapnpStructSchema:
        return load_versioned_schema("ee_trajectory_command.capnp").VersionedEETrajectoryCommand

    @classmethod
    @override
    def get_lcm_class(cls) -> type:
        return lcmt_ee_trajectory_command
    ...
    def _active_variant_field(self) -> attr.Attribute:
        for f in self._variant_fields():
            if getattr(self, f.name) is not None:
                return f
        raise InvalidDefinitionError("EETrajectoryCommand has no active variant")
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if len(active) != 1:
            raise InvalidDefinitionError(
                f"EETrajectoryCommand requires exactly one variant field to be set; got {len(active)}: {[f.name for f in active]}"
            )
    ...
    def from_capnp_v1(cls, reader: Any) -> Self:
        ...
        try:
            ...
            raise InvalidDefinitionError(f"Unknown ee_trajectory_command capnp arm {arm!r}") from e
    ...
    @override
    def to_lcm_message(self) -> lcmt_ee_trajectory_command:
        msg = lcmt_ee_trajectory_command()
    ...
    def from_lcm_message(cls, msg: Any) -> Self:
        ...
        if not (0 <= variant < len(variant_fields)):
            raise InvalidDefinitionError(f"Unknown ee_trajectory_command variant tag {variant}")
    ...
    def construct_default(cls) -> Self:
        return cls(
            ...
            ee_positions_trajectory=EEPositionsTrajectory.construct_default(),
        )
