...
EE positions are generalized coordinates: for a parallel gripper that's the
...
from manor.common.definitions.lcmtypes.lcmt_ee_positions import lcmt_ee_positions
...
@attr.frozen
class EEPositions(DefinitionBase):
    """
    EE generalized positions.
    """

    header: TimestampHeader
    positions: NpVectorNf64 = attr.field(
    ...
    def get_capnp_schema(cls) -> CapnpStructSchema:
        return load_versioned_schema("ee_positions.capnp").VersionedEEPositions

    @classmethod
    @override
    def get_lcm_class(cls) -> type:
        return lcmt_ee_positions
    ...
    @override
    def to_lcm_message(self) -> lcmt_ee_positions:
        msg = lcmt_ee_positions()
    ...
    def construct_default(cls, num_ee_dofs: int = 0) -> Self:
        return cls(header=TimestampHeader.construct_default(), positions=np.zeros(num_ee_dofs, dtype=np.float64))
