...
Every aegis config drives its own YAML key set and per-field parsing off its attrs definition; parse_attrs_yaml is the
canonical way to do that. It walks attr.fields(cls), reads each field's .type directly, and dispatches parsing off
that type:
...
* PEP 695 type aliases (TypeAliasType) -- unwrap to the underlying type and recurse.

Anything outside that set (numpy arrays, IManipulatorModel, DrakeLcm, tuple[T, ...] collections) is handled via the
custom_parsers / skip_fields escape hatches the helper exposes.

The primitive validators (require_int / require_number / require_bool / require_str / require_dict) keep value-type
checking uniform across configs so error messages and edge cases (notably bool being a subclass of int) are handled in
one place.
"""
...
def require_int(value: object, field_name: str) -> int:
    """
    Coerce a YAML scalar to int. Rejects bool explicitly because bool is a subclass of int in Python and would
...
def require_number(value: object, field_name: str) -> float:
    """
    Coerce a YAML scalar to float. Accepts both int and float source values; rejects bool for the same reason as
    require_int.
    """
...
def require_bool(value: object, field_name: str) -> bool:
    """
    Coerce a YAML scalar to bool -- strict on type so a stray 0 / 1 doesn't get accepted as a boolean flag.
    """
...
def require_str(value: object, field_name: str, *, allow_empty: bool = False) -> str:
    """
    Coerce a YAML scalar to str. allow_empty lets callers opt into accepting empty strings (e.g. an unset hardware
    serial_number).
    """
...
def require_dict(value: object, field_name: str) -> dict:
    """
    Coerce a YAML scalar to dict; rejects everything else with a helpful error pointing at field_name.
    """
...
def _parse_field_value(field_type: object, value: object, context: str) -> object:
    """
    Dispatch parsing of value off field_type (an actual class or generic alias). Recurses on X | None and on PEP 695
    type aliases.
    """
    ...
    # X | None / Optional[X]: pass through explicit null, otherwise recurse with the non-None branch.
    ...
    if isinstance(field_type, type):
        ...
        # Nested attrs config (or any class that knows how to parse itself from a YAML mapping). The base class for
        # tagged-union configs (MetisPolicyConfigBase / KyberControllerConfigBase) also implements this and dispatches
        # through its manager, which is what lets the helper recurse into them by field type alone.
...
def parse_attrs_yaml(
    ...
    *,
    ...
    """
    Walk the attrs fields of cls and parse the matching YAML keys. Returns a kwargs dict ready to splat into
    cls(**...).

    YAML keys must match attrs field names; unknown keys raise. ClassVar attributes (e.g. POLICY_TYPE, SYSTEM_NAME)
    are excluded automatically because attrs doesn't enumerate them as instance fields, which is exactly what we want
    here.

    Required-vs-optional follows attrs: a field with no default is required (missing keys raise AegisConfigError);
    fields with defaults / factories are simply omitted from the result when absent so the attrs default takes effect.
    """
    custom_parsers = custom_parsers or {}
    skip_fields = skip_fields or set()

    # from __future__ import annotations makes attr.field.type a string. resolve_types() rewrites those strings to
    # actual classes so the dispatch in _parse_field_value can use isinstance/issubclass directly. Idempotent; safe to
    # call repeatedly.
    try:
        attr.resolve_types(cls)
    except (NameError, AttributeError):
        # Forward refs that aren't importable from the class's module will still surface as a clear error inside
        # _parse_field_value.
        pass
