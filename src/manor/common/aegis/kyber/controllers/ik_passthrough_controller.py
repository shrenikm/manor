...
# Default time step for the differential IK QP. Diff IK is rate-controlled, so this is the dt used to
# convert v_next into the step's joint position increment for the joint-limit projection
# (q_current + N v_next dt). The aegis Kyber tick rate (typically 200 Hz) is the right ballpark; 5 ms is
# a safe default and is overridable via YAML.
_DEFAULT_DIFF_IK_TIME_STEP_S: float = 5e-3

# Joint position slack used when locking the EE block during the pose IK solve. For each non-arm DOF we
# constrain q to [measured - tol, measured + tol]; the slack only needs to absorb numerical noise, since
# IK is not allowed to move the EE here. Diff IK takes a stricter line and pins v[ee_block] to (0, 0)
# (see _build_diff_ik_params) so spatial-velocity tracking is allocated entirely to the arm DOFs.
_EE_BLOCK_POSITION_LOCK_TOL: float = 1e-4
_DEFAULT_ORIENTATION_THETA_BOUND_RAD: float = 1e-3

# Throttle keys for ManorLogger.warning_throttled. Each key buckets one class of warning so a sustained
# problem fires at most once per second per controller instance instead of flooding the terminal at the
# 500 Hz Kyber tick rate. Without these the IK-failure / unsupported-action paths look identical to
# "policy is doing nothing" and the operator can't tell the arm is wedged.
...
class IKPassthroughControllerConfig(KyberControllerConfigBase):
    """
    Config for IKPassthroughController. The controller derives plant geometry and frame names from
    manipulator_model; the YAML body only needs to override numerical knobs (diff IK time step,
    orientation tolerance for the IK pose solve) when the defaults aren't right for the task.
    """
...
class IKPassthroughController:
    """
    Cartesian-to-joint IK controller. Joint-shaped Action commands fall through unchanged;
    Cartesian-shaped Action commands go through Drake's InverseKinematics (pose) or
    DoDifferentialInverseKinematics (twist).
    """
    ...
    def _initialize_logger(self) -> ManorLogger:
        # Logger name is the class name so subclasses (if any are ever introduced) get their own logger
        # automatically without having to override anything.
    ...
    def build(
        ...
        # Cache the base + tip frames so every solve doesn't re-look them up by name. The base frame is
        # the explicit reference for every Cartesian target the controller accepts (pose translation,
        # pose orientation, twist linear / angular).
    ...
    def _build_diff_ik_params(self) -> DifferentialInverseKinematicsParameters:
        # Initialise with plant-derived position / velocity counts; joint position / velocity limits
        # come from the URDF via the plant. Pin the EE-block velocity limits to (0, 0) so diff IK never
        # allocates spatial-velocity tracking to non-arm joints.
        ...
        # Drake reports +/- inf on unlimited joints. Diff IK requires finite limits to stay well-posed;
        # clip to a wide-but-finite band on the arm DOFs and zero on the EE block.
        num_arm_dof = self.manipulator_model.get_num_dof()
        # rad/s -- much higher than realistic, just to keep the QP bounded.
        wide_arm_limit = 10.0
        ...
        # Same finite-band fix-up for joint position limits where the URDF leaves them unbounded.
        wide_q_limit = 10.0
    ...
    def _populate_plant_context(self, proprioception: Proprioception) -> None:
        ...
        # Proprioception's joint_state is sized to plant num_positions / num_velocities at steady
        # state; tolerate the early-bring-up case where it's still sized to 0 by leaving the context
        # at its default zero-q.
    ...
    def _solve_pose_ik(
        ...
        # Cartesian targets are explicitly in the manipulator base frame (see module docstring). Using
        # the named base frame rather than world_frame makes the contract obvious in the constraint set
        # and is robust against ever changing how the IK plant welds the manipulator into its world.
        ...
        # Drake's RotationMatrix takes a Quaternion(w, x, y, z) -- our CartesianPose stores orientation
        # in the same wxyz order.
        ...
        # Pin the EE block of q to its measured value via tight box bounds so IK only varies the arm
        # DOFs. Works generically: any plant DOF past num_arm_dof is treated as part of the EE, whatever
        # the URDF encodes there.
        ...
        if not result.is_success():
            # Degrade gracefully: hold current arm q. The diagram keeps ticking; the operator sees the
            # arm freeze rather than crash. Note the target is in the manipulator base frame (the IK
            # plant welds the base to the world origin) -- the most common cause of failure here is an
            # out-of-reach request expressed in the wrong frame.
    ...
    def _solve_twist_diff_ik(
        ...
        # Diff IK takes a 6-vector V = [angular; linear] paired with a spatial Jacobian; the V and the
        # Jacobian must agree on which frame they're expressed in. The controller's contract is
        # base-frame Cartesian targets (see module docstring), so the Jacobian uses the manipulator base
        # as both the "with-respect-to" and the "expressed-in" frame. For a base whose orientation
        # matches the world (e.g. lite6 with default base_rpy = [0, 0, 0]) this is numerically identical
        # to the world-frame version, but the contract is now explicit and robust to a non-trivial
        # mounting orientation later.
    ...
    def step(self, action: Action, proprioception: Proprioception) -> JointEECommand:
        ...
        if action.joint_command is not None:
            ...
            # JointTrajectoryCommand / CartesianTrajectoryCommand / EETrajectoryCommand: not handled in a
            # passthrough controller -- fall back to a zero-velocity arm command rather than crashing
            # the kyber tick. Trajectory-aware controllers should subclass / replace this.
