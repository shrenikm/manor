...
from manor.common.aegis.defaults import default_eef_state, default_joint_state
...
class TestSomaConstruction:
    ...
    def test_declares_expected_ports(self) -> None:
        ...
        assert soma.GetInputPort("joint_state") is not None
        assert soma.GetInputPort("eef_state") is not None
        assert soma.GetOutputPort("proprioception") is not None
...
class TestSomaProprioception:
    def test_assembles_proprioception_from_inputs(self) -> None:
        ...
        soma.GetInputPort("joint_state").FixValue(context, AbstractValue.Make(default_joint_state(num_joints=6)))
        soma.GetInputPort("eef_state").FixValue(context, AbstractValue.Make(default_eef_state(num_eef_dofs=1)))

        simulator = Simulator(soma, context)
        simulator.AdvanceTo(0.05)

        proprioception = soma.GetOutputPort("proprioception").Eval(simulator.get_context())
