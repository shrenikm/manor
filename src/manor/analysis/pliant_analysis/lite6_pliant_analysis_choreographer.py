...
from manor.lite6.pliant.lite6_pliant_utils import Lite6PliantConfig
from manor.lite6.utils.lite6_model_utils import LITE6_DOF, Lite6ControlType
            ...
            if self._section_end_wait_time is None:
                ...
                if self._current_section_ind == len(jcs) - 1:
                    if self._current_joint_ind == len(self.choreographer) - 1:
                        ...
                        joint_idx = joint_index + 1
                        section_idx = self._current_section_ind + 1
                        self._logger.info(
                            f"[Joint {joint_idx}][Section {section_idx}] "
                            "Current joint completed. Moving to the next one."
                        )
                    ...
                    section_idx = self._current_section_ind + 1
                    self._logger.info(
                        f"[Joint {joint_idx}][Section {section_idx}] Current section completed. Moving to the next one."
                    )
