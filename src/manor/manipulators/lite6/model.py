...
Per-variant data (description filename, MultibodyPlant position count) lives in module-level dicts keyed by
Lite6Variant. The arm DOF count and frame names are constant across all variants: the Lite6 arm always has 6
joints, and every supported variant uses link_base as the base frame and link_eef_tip as the FK / IK
reference frame.
"""
...
LITE6_ARM_DOF = 6
# Number of EE joints in the Drake plant (URDF) for the parallel gripper variants: two prismatic finger
# joints with opposite signed travel (left in [0, +0.008], right in [-0.008, 0]).
LITE6_PARALLEL_GRIPPER_PLANT_DOF = 2
# Number of EE-level DOFs surfaced through IManipulatorModel for the parallel gripper variants. The EE vector
# is the opening width (one scalar) -- one EE DOF, not two.
LITE6_PARALLEL_GRIPPER_EE_DOF = 1

# Joint travel per finger (URDF axis range), shared across the normal and reverse parallel-gripper variants.
# 8 mm per finger; both fingers combined cover 16 mm of joint-space travel.
_LITE6_PARALLEL_FINGER_HALF_TRAVEL_M: float = 0.008

# URDF link-origin offset of each finger from the gripper centre line at q = 0. The two parallel-gripper
# variants differ ONLY in this constant: in normal mounting the finger heads point toward each other (small
# offset, jaws nearly touch at q=0); in reverse mounting the finger heads are unscrewed and flipped, mounting
# outward (larger offset, jaws have a built-in 27 mm gap at q=0). The joint travel is identical, so reverse
# mode reaches a wider physical opening.
#
# The EE-level "opening width" we surface to policies / controllers is the PHYSICAL jaw gap, which equals
# (left - right) + 2 * origin_offset. That makes width semantics consistent across the two mountings: the
# user always commands a real-world distance, and the per-variant offset table absorbs the URDF geometry
# difference.
...
# Physical jaw-gap width range per variant. Closed = q at (0, 0) (smallest physical gap reachable in this
# mounting); open = q at (+travel, -travel) (largest gap). The numeric values track the URDF link-origin
# offsets above.
...
# Per-variant EE-position limits surfaced through IManipulatorModel.get_ee_position_limits. Lower is the
# smaller physical-width bound for the parallel-gripper variants and the "off" end of the binary range for
# vacuum; upper is the larger. Open / closed semantics are NOT encoded here -- those are policy-side
# conventions.
...
# MultibodyPlant position counts. The vacuum gripper has no actuated DOFs of its own, so its plant has just
# the 6 arm joints. The actuated parallel grippers add two prismatic finger joints.
...
# EE generalized-DOF counts surfaced through the IManipulatorModel interface (i.e. the size of EEPositions /
# EEVelocities vectors for this variant). The vacuum gripper exposes a single binary on/off state; both
# parallel-gripper variants expose a single opening width that the model splits internally across the two
# prismatic finger joints in the URDF.
...
class Lite6Model(IManipulatorModel):
    ...
    def get_num_velocities(self) -> int:
        # All Lite6 joints (arm + parallel gripper) are simple revolute / prismatic, so num_velocities equals
        # num_positions.
        return _VARIANT_TO_NUM_POSITIONS[self.variant]
    ...
    def ee_positions_to_plant_positions(self, ee_positions: EEPositionsVector) -> PlantEEPositionsVector:
        # Vacuum: the URDF has no actuated EE joints, so the EE binary on/off lives entirely in the
        # proprioception channel and never touches the plant's q vector.
        if self.variant is Lite6Variant.VACUUM_GRIPPER:
            return np.zeros(0, dtype=np.float64)
        # Parallel gripper: the EE vector is the single physical jaw opening width in metres. Translate to
        # URDF q space by subtracting the variant-specific built-in gap (the URDF link origin offset doubled
        # across the two fingers), then split the remaining travel symmetrically across the two prismatic
        # joints. See the module-level constants for the per-variant offset and the URDF link-origin
        # provenance.
    ...
    def plant_positions_to_ee_positions(self, plant_ee_positions: PlantEEPositionsVector) -> EEPositionsVector:
        # Vacuum gripper has no actuated EE joints in the plant, so the plant block is empty and there's no q
        # to project from. With no separate latch path here, the best the sim can report is "off" (zero); the
        # binary state is supplied by the EE-command channel, not derived from physics.
        if self.variant is Lite6Variant.VACUUM_GRIPPER:
        ...
        # Physical width = (left - right) + 2 * origin_offset. The second term is the constant gap baked
        # into the URDF geometry (and the only thing that distinguishes normal from reverse).
        offset = _VARIANT_TO_FINGER_LINK_ORIGIN_OFFSET_M[self.variant]
    ...
    def ee_velocities_to_plant_velocities(self, ee_velocities: EEVelocitiesVector) -> PlantEEVelocitiesVector:
        ...
        if self.variant is Lite6Variant.VACUUM_GRIPPER:
            return np.zeros(0, dtype=np.float64)
        # Parallel gripper: the URDF-origin offset is constant, so it drops out of the time derivative --
        # this is pure half-and-mirror, identical for normal and reverse mountings.
    ...
    def _plant_positions_at(self, configuration: Lite6JointConfiguration) -> JointPositionsVector:
        ...
        # For the parallel-gripper variants the two prismatic fingers at q=0 sit at the URDF link-origin
        # offset, which is the variant's "closed" width -- a reasonable default to start in. Vacuum variants
        # have no actuated EE joints, so the EE block is empty.
    ...
    def get_default_sim_pid_gains(self) -> PIDGains:
        # Tuned in the previous (deprecated) Lite6 sim against the choreographer analysis plots. Arm joints
        # carry mid-range gains; the parallel-gripper fingers run much stiffer (the 5 g finger links
        # accelerate fast under gravity without damping declared on those prismatic joints).
