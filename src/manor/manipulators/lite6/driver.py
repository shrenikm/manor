...
Wraps XArmAPI to satisfy IManipulatorDriver. The bring-up / tear-down sequences (prime / unprime) live in
manor.manipulators.lite6.xarm_helpers so this driver and the standalone lite6_cli share a single validated
implementation.

End-effector read/write semantics on the Lite6 are limited by the SDK:

* The SDK exposes joint-only state (the 6 arm positions / velocities); gripper position is not readable. Both
  read_ee_* methods return None.
* Gripper actuation is binary (open / close / stop). write_ee_* methods threshold the supplied EE vector to
  map onto those calls.

The driver does NOT disconnect on unprime -- motors stay energized and the TCP session stays open so
subsequent re-prime cycles (e.g. after a stale-command watchdog trip in HardwareManipulatorBackend) skip the
brake-release / encoder-relock latency. The session is released only when the process exits.
"""
...
# The xarm SDK prints SDK_VERSION: <ver> to stdout on import, with no off switch. Swallow it with a stdout
# redirect during the import so every aegis CLI / runner invocation isn't preceded by that banner. xarm is a
# hard dependency (pyproject.toml) -- if the import fails, that's a real environment problem and we want the
...
# Heuristic threshold for collapsing a continuous EE position vector onto the parallel gripper's binary open
# / close command. The parallel-gripper URDFs use ~0.008 m for "open"; halfway is fine.
...
class Lite6Driver(IManipulatorDriver):
    """
    IManipulatorDriver implementation that talks to a real Lite6 over the xarm SDK.
    """
    ...
    # Sticky-mode cache. Source of truth for the current operating mode -- the firmware's heartbeat-cached
    # arm.mode lags by ~200 ms and isn't reliable to compare against. None until prime() runs; prime / unprime
    # always end in mode POSITION (mode 0) so we set it to that. write_joint_positions /
    # write_joint_velocities lazily switch into SERVO_POSITION (mode 1) / VELOCITY (mode 4) on first use and
    # on every shape transition; characterised cost is ~190-210 ms per switch (mostly the heartbeat-poll
    # inside switch_mode), so per-tick switching is infeasible -- caching is what makes mixed-shape policies
    # tolerable.
    _current_mode: XArmMode | None = attr.field(init=False, default=None)
    # Client-side rate limiter state for write_joint_positions. The xarm SDK's per-call speed= argument is
    # ignored by the firmware for set_servo_angle_j (mode 1 streaming), so we clamp the advancement of the
    # commanded position ourselves. The previously commanded (position, timestamp) pair lives inside a single
    # JointPositions -- the header carries the monotonic_ns we need for dt, the positions field carries the
    # vector we step from. None until the first write after prime / resume / unprime / a mode change.
    ...
    def _initialize_arm(self) -> XArmAPI:
        # do_not_open=True keeps the constructor cheap and side-effect free: no socket is opened until prime()
        # runs. Without this, instantiating the driver in a test (or in any code path that imports the
        # hardware backend without intending to actually drive an arm) would synchronously try to reach the
        # controller IP and fail. The first prime() call connects + brings the arm up.
    ...
    @override
    def prime(self) -> None:
        # Bring-up sequence shared with lite6_cli: clean errors, motion_enable, settle, mode 0 / READY,
        # soft-recover on latched servo errors, move to PRIME. Always leaves the arm in mode POSITION (mode
        # 0). The XArmAPI was constructed with do_not_open=True, so open the TCP session before issuing SDK
        # calls. arm.connect() is cheap if already open (we guard on arm.connected anyway).
        try:
    ...
    def unprime(self) -> None:
        ...
        # motion_enable(False), does NOT disconnect the TCP session, and does NOT null self._arm -- the
        # watchdog / aegis-shutdown paths may re-prime on the same driver instance. xarm_unprime always
        # finishes by setting mode POSITION, so reset the cache to match.
        try:
    ...
    def _ensure_mode(self, target: XArmMode) -> None:
        # Sticky-mode gate for the streaming write paths. switch_mode goes through STOP -> set_mode -> READY
        # and waits for the heartbeat to confirm; expensive (~200 ms) and aborts any in-flight motion, so we
        # only call it on transitions. Mid-motion switches are safe (the embedded set_state(STOP) decelerates
        # cleanly, no faults), characterised on hardware on 2026-05-04. _current_mode is None only before
        # prime(); a write before prime is a usage error -- raise rather than silently driving a not-ready
        # arm. Any actual mode change also resets the position limiter -- the previously commanded streaming
        # target is stale if motion went through a different mode in between (e.g. a VELOCITY phase moved the
        # arm away from the last commanded JointPositions), so we re-seed from measured on the next
    ...
    @override
    def halt(self) -> None:
        # Refuse motion at the controller level without moving or changing mode -- the arm holds its current
        # pose, motors stay energized, mode stays as set by prime. The watchdog calls this when the action
        # stream goes stale; resume() flips state back to READY.
        self._call(self._arm.set_state(state=XArmState.STOP), "set_state(stop)")

    @override
    def resume(self) -> None:
        # Inverse of halt. set_state(READY) alone is NOT sufficient to re-arm the firmware's streaming
        # controller after halt's set_state(STOP): characterised on hardware on 2026-05-04, after a
        # halt/resume cycle subsequent set_servo_angle_j / vc_set_joint_velocity calls return code=0 but the
        # firmware silently ignores the streamed target. The full switch_mode dance (STOP -> set_mode ->
        # READY) is what actually re-arms streaming, so we re-flip the cached mode here even though the
        # firmware-side mode value hasn't changed. _current_mode is None only if resume runs before prime()
        # ever did (unusual; the watchdog can't trip pre-prime, but defensive). Also reset the position
        # limiter -- the arm decelerated under halt so its actual pose may differ from the last commanded
        # JointPositions, and re-seeding from measured on the next write avoids a snap when streaming
        # resumes.
        if self._current_mode is None:
            self._call(self._arm.set_state(state=XArmState.READY), "set_state(ready)")
        else:
            try:
                xarm_switch_mode(self._arm, mode=self._current_mode, log_fn=self._logger.info)
            except XArmCallError as exc:
                raise Lite6DriverError(str(exc)) from exc
        self._reset_position_limiter()
    ...
    def _read_joint_state(self) -> tuple[JointPositionsVector, JointVelocitiesVector]:
        ...
        try:
            ...
            raise Lite6DriverError(str(exc)) from exc
        # The SDK returns 7-element vectors regardless of arm DOF; the 7th value is auxiliary (used on 7-DOF
        # models). Slice to the arm's actual DOF count.
    ...
    def _now_timestamp_header(self) -> TimestampHeader:
        ...
        # global time module (which would break TimestampHeader.from_system_time and every other monotonic_ns
        # reader running in the same process).
    ...
    def _compute_rate_limited_target(self, target: JointPositionsVector) -> JointPositionsVector:
        # On the first call after prime / unprime / resume / a mode change the limiter has no history, so we
        # seed it from the arm's measured pose and emit that as the commanded value. The first SDK call is
        # then a no-op write (commanded == measured) which aligns the streaming target with where the arm
        # actually is; subsequent calls advance toward the policy target at the configured speed.
    ...
    def write_joint_positions(self, joint_positions: JointPositions) -> None:
        # set_servo_angle_j requires mode SERVO_POSITION (mode 1). Switch lazily on the first joint-position
        # write after prime / a velocity write; subsequent same-shape writes are a cheap cache hit. The
        # commanded angles are clamped client-side via _compute_rate_limited_target so each call advances by
        # at most joint_speed_limit_rad_s * dt from the previously commanded position; the SDK's own speed=
        # argument is reserved/ignored by the firmware for this call so we don't pass it.
    ...
    def write_joint_velocities(self, joint_velocities: JointVelocities) -> None:
        ...
        # write_joint_positions; the arm's controller silently no-ops vc_set_joint_velocity if the mode is
        # wrong, which is exactly the bug that motivated this whole refactor.
    ...
    def write_ee_positions(self, ee_positions: EEPositions) -> None:
        # Map the continuous EE position vector onto the SDK's binary open / close command via a threshold on
        # the max element.
    ...
    def _call(self, ret_code: int | tuple, op: str) -> None:
        # Thin adapter around the shared check_xarm_call so driver callers raise Lite6DriverError (the
        # project-specific exception) rather than the generic XArmCallError the helpers speak in.
        try:
