...
class Lite6GripperStatus(StrEnum):
    OPEN = "open"
    CLOSED = "closed"
    NEUTRAL = "neutral"
...
def get_positions_from_lite6_state(
    ...
    """
    Returns all positions (joint + gripper) from the state vector.
    """
    assert lite6_model_type in Lite6ModelGroups.LITE6_ROBOT_MODELS
...
def get_joint_positions_vector_from_lite6_state_vector(
    ...
    """
    Returns only the arm joint positions (excludes gripper positions).
    """
    assert lite6_model_type in Lite6ModelGroups.LITE6_ROBOT_MODELS
...
def get_velocities_from_lite6_state(
    ...
    """
    Returns all velocities (joint + gripper) from the state vector.
    """
    assert lite6_model_type in Lite6ModelGroups.LITE6_ROBOT_MODELS
    nq = get_lite6_num_positions(lite6_model_type=lite6_model_type)
    return state_vector[nq:]
...
def get_default_lite6_positions_vector(
    lite6_model_type: Lite6ModelType,
) -> PositionsVector:
    """
    Returns all default positions (joint + gripper) for the given model type.
    For actuated gripper models, the default gripper state is closed.
    """
    assert lite6_model_type in Lite6ModelGroups.LITE6_ROBOT_MODELS
    state_vector = np.zeros(get_lite6_num_states(lite6_model_type), dtype=np.float64)
...
def get_default_lite6_joint_positions_vector(
    ...
    """
    Returns only the default arm joint positions (excludes gripper).
    """
    assert lite6_model_type in Lite6ModelGroups.LITE6_ROBOT_MODELS
    state_vector = np.zeros(get_lite6_num_states(lite6_model_type), dtype=np.float64)
