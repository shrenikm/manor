...
from manor.manipulators.deprecated_lite6.utils.lite6_model_utils import LITE6_DOF, Lite6ControlType, Lite6GripperStatus
...
class Lite6HardwareInterface(LeafSystem):
    def __init__(
        ...
        self._logger = ManorLogger(self.__class__.__name__)
        ...
        if XArmAPI is None:
            self._logger.warning(
                "xarm-python-sdk is not installed. Hardware pliant will be mocked. "
                "Install with: pip install xarm-python-sdk"
            )
    ...
    def _send_commands(self, context: Context) -> EventStatus:
        ...
        if self.config.lite6_control_type == Lite6ControlType.STATE:
            ...
            if ret_code != 0:
                self._logger.warning("Failed to set joint state.")
        elif self.config.lite6_control_type == Lite6ControlType.VELOCITY:
            ret_code = (
            ...
            if ret_code != 0:
                self._logger.warning("Failed to set joint velocities.")
        else:
            raise NotImplementedError("Invalid control type")
    ...
    def _compute_estimated_state_output(
        self,
        context: Context,
        ...
        output_value.set_value(
            ManipulatorState(
                joint_positions=state_estimated[:LITE6_DOF],
                joint_velocities=state_estimated[LITE6_DOF:],
